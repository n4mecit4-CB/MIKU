...
/*------------------------------------------------*/
		...
		if (!isNumber(user.IDregister)) user.IDregister = 0   
	      ...
	      if (!isNumber(user.lastpago)) user.lastpago = 0 
            ...
            if (chat) {
                ...
                if (!('sWelcome' in chat)) chat.sWelcome = '爪丨Ҝㄩ'                    
                if (!('sBye' in chat)) chat.sBye = 'NO SOPORTÓ 🤣 QUE LE VALLA BONITO 🫶'                    
		...
		if (!('autolevelup' in chat))  chat.autolevelup = false
                ...
                global.db.data.chats[m.chat] = {
                    ...
                    detect: true,
                    sWelcome: '',
                    sBye: 'NO SOPORTO 🤣 QUE LE VALLA BONITO 🫶',
                    sPromote: '',
                    sDemote: '', 
                    delete: true,
                    modohorny: true,
...
user.commandCount = 1;
               ...
               if (plugin.rowner && plugin.owner && !(isROwner || isOwner)) { // Both Owner
                ...
                if (!isPrems && plugin.money && global.db.data.users[m.sender].money < plugin.money * 1) {
                    this.reply(m.chat, `🐈 𝙉𝙊 𝙏𝙄𝙀𝙉𝙀 MIKUCOINS`, m)
                    continue     
		}
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '😈 爪丨Ҝㄩ') :
...
"thumbnail": apii.data, 
"title": [wm, '😈 𝗦𝘂𝗽𝗲𝗿 ' + gt + ' 😈', '🌟 n4mecit4bot.gmail.com'].getRandom(),
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '😈 𝗦𝘂𝗽𝗲𝗿 ' + gt + ' 😈', '🌟 n4mecit4bot.gmail.com'].getRandom(), thumbnail: gataImg.getRandom(), sourceUrl: [md, nna, nn, nnn, nnnt, nnnttt4, yt, ig, paypal, fb].getRandom() }}}}, tg)
