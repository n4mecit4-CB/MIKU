...
const smsRowner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO YO COMO CREADOR(A) DE BOT LO PUEDO USAR!!\`\`\`` }//NUMERO DE BOT
...
const smsBotAdmin = () => { return `\`\`\`¡¡NECESITO SER ADMIN PARA QUE PUEDA USAR ESTE COMANDO!!\`\`\`` }//BOT CON ADMIN
...
const smsLista1 = () => { return `🌟 INFORMACIÓN DE 爪丨Ҝㄩ 🌟`}
...
const smsLista7 = () => { return `🐈 INSTALAR 爪丨Ҝㄩ 🐈`}
...
const smsBye = () => { return '*╭┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*\n┊ @user\n┊ *NO SOPORTÓ PANZON@, CHAO!!* 😎\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*'}
const smsSpromote = () => { return '*@user 𝘼𝙃𝙊𝙍𝘼 𝙀𝙎 𝘼𝘿𝙈𝙄𝙉 𝙀𝙉 𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊, CUIDA TU PUESTO!!*'}
const smsSdemote = () => { return '*@user 𝘿𝙀𝙅𝘼 𝘿𝙀 𝙎𝙀𝙍 𝘼𝘿𝙈𝙄𝙉 𝙀𝙉 𝙀𝙎𝙏𝙀 𝙂𝙍𝙐𝙋𝙊, TE LO PERSISTE, CHAO!!*'}
...
//_allantilink.js
const smsTextoYT = () => { return '😻 𝗦𝘂𝗽𝗲𝗿 爪丨Ҝㄩ - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
...
const smsModP2 = () => { return `Habilitar función para que todos puedan usar 爪丨Ҝㄩ`}
const smsModAd1 = () => { return `🛂 MODO ADMIN`}
const smsModAd2 = () => { return `Solo los Admins podrán usar CarliWiiz en Grupos`}
...
//grupo-add.js
const smsAddB1 = () => { return `${lenguajeGB['smsAvisoFG']()}*NO SE PUEDE AGREGAR EL NÚMERO, VERIFIQUE QUE SEA CORRECTO, TAL VEZ SALIÓ RECIENTEMENTE O SU PRIVACIDAD ESTA CONFIGURADA.*`}
const smsAddB2 = () => { return `${lenguajeGB['smsAvisoFG']()}*NO SE PUEDE AGREGAR EL NÚMERO, VERIFIQUE QUE SEA CORRECTO, O AGRÉGELO MANUALMENTE.*`}
...
const smsContacto4 = () => { return 'Contacto Oficial de 爪丨Ҝㄩ ✨'}
